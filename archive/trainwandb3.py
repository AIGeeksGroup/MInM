...
def get_args_parser():
    ...
    parser.add_argument('--warmup_epochs', type=int, default=120, metavar='N',
...
def write_progress(epoch, status, acc):
    """ 更新 progress.txt """
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 10 == 4 or epoch + 1 == args.epochs:
            while True:
                last_epoch, status, acc = read_progress()
                if status == "probing_done":
                    break
                if epoch == 4:
                    acc = 0.0
                    break
                print(f"Waiting for engine_probing to finish epoch {epoch - 1}...")
                time.sleep(10)
            ...
            write_progress(epoch,"model_saved",acc)

            if misc.is_main_process():
                print(f"Current best accuracy after linear probing: {acc:.2f}%")
