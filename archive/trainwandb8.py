...
# --------------------------------------------------------
# mainpretrain:no_distributed
# linearprobing:no_distributed
...
from engine.engine_probing import linear_probing
...
class MaskedImageDataset(Dataset):
    def __init__(self, image_root, mask_root, transform=None):
        """
        image_root: 训练集的根目录，包含多个子目录（类别）
        mask_root: Mask 的根目录，包含相应的子目录（类别）
        transform: 图像变换
        """
        self.image_paths = []
        self.mask_paths = []
        self.class_indices = []  # 记录类别索引
        self.transform = transform if transform else transforms.ToTensor()

        class_folders = sorted(os.listdir(image_root))  # 获取所有类别子文件夹
        print(f"检测到 {len(class_folders)} 个类别: {class_folders}")
        ...
        for class_index, class_name in enumerate(class_folders):
            ...
            for img_name in images:
                ...
                mask_name = img_name.replace(".JPEG", "_mask_applied.png")  # 确保 mask 匹配
                ...
                if os.path.exists(mask_path):
                    ...
                    print(f"⚠️ 警告: 未找到 {mask_path} 对应的 mask！")

        print(f"✅ 加载 {len(self.image_paths)} 张图片和 {len(self.mask_paths)} 张 mask")
    ...
    def __getitem__(self, idx):
        ...
        mask = Image.open(mask_path).convert("L")  # Mask 是灰度图
        class_idx = self.class_indices[idx]

        # 先转换 mask 为 Tensor
        mask = TF.to_tensor(mask)

        # 确保 mask 形状正确（如果是单通道，扩展为 3 通道）
        if mask.shape[0] == 1:
        ...
        if self.transform:
            ...
            # 处理 mask，跳过 ToTensor 变换
            for t in self.transform.transforms:
                if isinstance(t, torch.nn.Module):
                    mask = t(mask)  # 只对 mask 继续应用其他变换（如 Normalize）

        return image, mask_path, class_idx
...
def main(args):
    ...
    dataset_train = MaskedImageDataset(
        ...
        mask_root=os.path.join(args.data_path, "mask/masks_applied"),
    ...
    if len(dataset_train) == 0:
        raise ValueError("❌ 错误！数据集为空，请检查数据路径是否正确。")
