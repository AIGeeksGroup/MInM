# MAE baseline (75% random masking) on imagenette — control experiment
# Compares against MInM (instance-guided masking) on the same data
# Usage: python tools/train_imagenette_mae.py [--epochs N] [--batch_size N]
...
class MAERandomMaskDataset(torch.utils.data.Dataset):
    """Dataset for vanilla MAE: loads images and generates random patch-level masks.
    Returns (image, mask, class_idx) to match engine_pretrain_minm's expected format.
    mask: [num_patches] binary tensor, 1=masked, 0=kept, with ~75% masked.
    """
    def __init__(self, image_root, mask_ratio=0.75, patch_size=16, img_size=224):
        ...
        for class_idx, class_name in enumerate(class_folders):
            ...
            for img_name in images:
                self.image_paths.append(os.path.join(image_class_dir, img_name))
                self.class_indices.append(class_idx)

        print(f"MAE baseline: loaded {len(self.image_paths)} images from {image_root}")
    ...
    def __getitem__(self, idx):
        ...
        # Generate random mask: 1=masked, 0=kept
        num_mask = int(self.num_patches * self.mask_ratio)
        mask = torch.zeros(self.num_patches)
        mask_indices = torch.randperm(self.num_patches)[:num_mask]
        mask[mask_indices] = 1.0

        return image, mask, self.class_indices[idx]


def get_args_parser():
    parser = argparse.ArgumentParser('MAE baseline (random masking) on imagenette', add_help=False)
    ...
    parser.add_argument('--output_dir', default='output_mae_imagenette', type=str)
    parser.add_argument('--log_dir', default='output_mae_imagenette', type=str)
...
def main(args):
    ...
    # MAE baseline: no SAM masks, just random masking
    dataset_train = MAERandomMaskDataset(
    ...
    # Model setup — use_instance_mask=True so forward_encoder uses the passed mask
    # (which is now a random mask instead of instance mask)
